...
type RayOrigin = 'top-left' | 'top-center' | 'top-right' | 'left' | 'right' | 'bottom-left' | 'bottom-center' | 'bottom-right';
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  // Boost intensity for bright white glow (kept moderate for Safari compatibility)
  fragColor.rgb = clamp(fragColor.rgb * 2.2, 0.0, 1.0);
  // Premultiply alpha for correct screen blending
...
const LightRaysCanvas = ({
  ...
  const baseAnchorRef = useRef<number[]>([0, 0]);
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const loop = (t: number) => {
        ...
        const mouseOffsetX = (smoothMouseRef.current.x - 0.5) * res[0] * 0.4;
        uniforms.rayPos.value = [baseAnchorRef.current[0] + mouseOffsetX, baseAnchorRef.current[1]];
        try {
...
const LightRaysOverlay = (props: LightRaysProps) => {
  const location = useLocation();
  if (location.pathname.startsWith('/admin')) return null;
  return <LightRaysCanvas {...props} />;
};
