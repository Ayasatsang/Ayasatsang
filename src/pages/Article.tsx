  ...
  return (
    <Link to={`/articles/${article.slug || article.id}`} className="cursor-pointer group w-full block">
      ...
      <div className="flex flex-col justify-between article-card-content">
        ...
        {/* Tags */}
        <div className="hidden lg:flex flex-wrap article-card-tags">
...
const ArticlePage = () => {
  ...
  // Find current article by id or slug
  const articleData = articles.find(a => a.id === id || a.slug === id) as ArticleData | undefined;

  // Get related articles (exclude current, only published)
  const relatedArticles = articles
    .filter(a => a.id !== articleData?.id && a.status === 'published')
    .slice(0, 3) as ArticleData[];
    ...
    return (
      <div className="relative" style={{ background: '#73BEEC', minHeight: '100vh', overflow: 'clip' }}>
        ...
        <div className="flex items-center justify-center" style={{ paddingTop: '200rem' }}>
          <p style={{ fontSize: '24rem', color: '#fff' }}>Стаття не знайдена</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <EditorProvider pageId="article">
    <Helmet>
      <title>{articleData.metaTitle || articleData.title} | Айа</title>
      {(articleData.metaDescription || articleData.description) && (
        <meta name="description" content={articleData.metaDescription || articleData.description} />
      )}
      <meta property="og:title" content={articleData.metaTitle || articleData.title} />
      {(articleData.metaDescription || articleData.description) && (
        <meta property="og:description" content={articleData.metaDescription || articleData.description} />
      )}
        ...
        <div className="relative z-10 flex flex-col items-center article-hero-content">
          ...
          <div className="flex items-center justify-center flex-wrap article-hero-meta">
            ...
            <span className="article-hero-meta-text">
              2 хв читання · {formatDate(articleData.date)}
            </span>
          </div>
        </div>
      </section>
        ...
        <section className="flex justify-center article-main-image-section">
          <div className="relative article-main-image-container">
            <img
              ...
              onError={(e) => {
                (e.target as HTMLImageElement).src = 'https://placehold.co/808x404/D4E9F7/6AB4FF?text=Article+Image';
              }}
            />
          </div>
        </section>
        ...
        {articleData.description && (
          <section className="flex justify-center article-description-section">
            <p className="text-center article-description-text" style={{ whiteSpace: 'pre-line' }}>
              {(articleData.description || '').replace(/\\n/g, '\n')}
            </p>
          </section>
        )}
